...
export const useCommandPalette = ({
  ...
  useEffect(() => {
    const saved = localStorage.getItem('savify_search_history');
    if (saved) {
      try {
  ...
  // Search for music when query changes using Jamendo (only when not using mock data)
  const useMockData = shouldUseMockData();
  const {
  ...
  // Transform music search results (from either Jamendo API or mock search)
  const musicResults: SearchResult[] = useMemo(() => {
    if (useMockData) {
      ...
      // Use Jamendo API results
      return musicSearchData.results.slice(0, 8).map((track: ITrack) => ({
        id: track.id,
        type: 'track' as const,
        title: track.title || track.name || 'Unknown Track',
        subtitle: `${track.artist || 'Unknown Artist'} • ${track.album || 'Unknown Album'}`,
  ...
  const handleItemSelect = (item: SearchResult) => {
    ...
    if (query.trim()) {
      ...
      localStorage.setItem('savify_search_history', JSON.stringify({
        queries: newSearches,
        items: newHistory
      }));
    }
  ...
  // Loading state (handles both mock search and Jamendo API)
  const isLoading = useMockData
    ? isMockSearchLoading && query.trim()
    : isMusicSearchLoading && query.trim();

  // Error state (only show errors for Jamendo API, mock search shouldn't fail)
  ...
  return {
    ...
    clearHistory: () => {
      setRecentSearches([]);
      setSearchHistory([]);
      localStorage.removeItem('savify_search_history');
    }
  };
};